<section class="table-wrapper" style = "padding-top: 1em; padding-bottom: 1em">
    ...
    {if {$numRecords} > 0}
        ...
        <form method="post">
            ...
            <center>    
                <button class="primary" onclick="ajaxPostForm('form', '{url action="borrowedListData" p = 1}', 'data'); return false;" {if {$page}==1}disabled{/if}> &lt;&lt; </button>
                <button class="primary" onclick="ajaxPostForm('form', '{url action="borrowedListData" p = {$page-1}}', 'data'); return false;" {if {$page-1}==0}disabled{/if}> &lt; </button>
                <span style="margin:5%">Strona {$page} z {$lastPage-1}</span>
                <button class="primary" onclick="ajaxPostForm('form', '{url action="borrowedListData" p = {$page+1}}', 'data'); return false;" {if {$page+1}=={$lastPage}}disabled{/if}> &gt; </button>
                <button class="primary" onclick="ajaxPostForm('form', '{url action="borrowedListData" p = {$lastPage-1}}', 'data'); return false;" {if {$page}=={$lastPage-1}}disabled{/if}> &gt;&gt; </button>
            </center>
        </form>
